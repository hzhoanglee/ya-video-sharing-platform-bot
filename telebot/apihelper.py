...
def _make_request(token, method_name, method='get', params=None, files=None):
    ...
    if API_URL:
        ...
        request_url = "http://127.0.0.1:8999/bot{0}/{1}".format(token, method_name)
...
def get_file_url(token, file_id):
    if FILE_URL is None:
        return "http://127.0.0.1:8999/file/bot{0}/{1}".format(token, get_file(token, file_id)['file_path'])
    else:
        # noinspection PyUnresolvedReferences
...
def download_file(token, file_path):
    if FILE_URL is None:
        url =  "http://127.0.0.1:8999/file/bot{0}/{1}".format(token, file_path)
    else:
        # noinspection PyUnresolvedReferences
        url =  FILE_URL.format(token, file_path)

    # return content of the file as it is local from file_path
